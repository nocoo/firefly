...
describe("getAgentAvatarUrl", () => {
  it("builds correct URL for a given agent ID, version, and size", () => {
    expect(getAgentAvatarUrl("01HQ1234567890", "v1", 64)).toBe(
      "https://test-cdn.example.com/uploads/firefly/agents/01HQ1234567890/v1/avatar-64.jpg",
    );
  });
  ...
  it("builds correct URL for all standard sizes", () => {
    ...
    for (const size of sizes) {
      ...
      expect(url).toContain(`/avatar-${size}.jpg`);
...
describe("getAgentAvatarR2Key", () => {
  it("builds R2 key without CDN prefix using agent ID", () => {
    expect(getAgentAvatarR2Key("01HQ1234567890", "v1", 64)).toBe(
      "uploads/firefly/agents/01HQ1234567890/v1/avatar-64.jpg",
    );
  });
...
describe("getAllAvatarR2Keys", () => {
  it("returns keys for all 4 avatar sizes using agent ID", () => {
    ...
    expect(keys).toContain("uploads/firefly/agents/01HQ1234567890/v1/avatar-32.jpg");
    expect(keys).toContain("uploads/firefly/agents/01HQ1234567890/v1/avatar-64.jpg");
    expect(keys).toContain("uploads/firefly/agents/01HQ1234567890/v1/avatar-128.jpg");
    expect(keys).toContain("uploads/firefly/agents/01HQ1234567890/v1/avatar-256.jpg");
  });
});
