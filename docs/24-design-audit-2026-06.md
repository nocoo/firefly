...
## 执行记录 — 第二波（2026-06-09 完成）

「6.2 第二波：primitive 抽取」全部 13 项落盘 + 1 文档 commit = **14 个原子化提交**，全部 pre-commit（typecheck + lint + 1379 单测）。Coverage 100% statements / 99.59% branches / 100% functions / 100% lines（无回归，新增 primitive 都进 `src/components/ui/**` 或 `src/components/blog/**`，已在 vitest exclude 列表，不计入 coverage 但所有调用方仍跑测试）。

| 项 | 状态 | Commit | 改动摘要 |
|----|------|--------|----------|
| **#1 Textarea primitive** | ✅ | `f014186` | `ui/textarea.tsx`，对齐 Input 的 token 风格 |
| **#2 PostForm 用 Textarea** | ✅ | `e3081fe` | `post-form-content-editor` write/preview 模式 + `post-form-fields` excerpt 三处 |
| **#3 Admin 其余 textarea** | ✅ | `3d2e296` | `site-identity-form`、`ai-agent-form`、`post-form-reference-fields` |
| **#4 Input 替换 raw** | ✅ | `e00e4f1` | `taxonomy-manager-form`（含 Button 替换）、`mcp-tokens-create-form`、`post-form-fields` featured-image、`post-form-reference-fields` URL+title |
| **#5 EmptyState 提到 ui/** | ✅ | `28bd10f` | `ui/empty-state.tsx` 加 `action`/`secondaryAction` slot 和 `variant` blog/admin；blog 版改 re-export |
| **#6 Blog 列表空态加 action** | ✅ | `ef95e3f` | category × 2、tag × 2、archive × 2、search × 2 共 8 处加"返回首页"action 解 B-12 |
| **#7 Admin 空态用 EmptyState** | ✅ | `4fdaccc` | `mcp-tokens-manager`、`admin-posts-grid-view` |
| **#8 FormField wrapper** | ✅ | `8559380` | `ui/form-field.tsx` label + description + error，aria-描述链路 |
| **#9 PostForm 用 FormField** | ✅ | `94cb84b` | title/slug 字段从手写 `<div>` 收敛到 FormField + Input；error 状态 `border-destructive` |
| **#10 FeaturedImage 组件** | ✅ | `181d95b` | `blog/featured-image.tsx` + `FEATURED_IMAGE_SIZES` 常量 |
| **#11 三处 Image 接入 FeaturedImage** | ✅ | `bec318d` | `[slug]` + `preview/[id]` 用组件；`post-card.tsx`（Link overlay 结构特殊）只换 sizes 常量，仍调用 `FEATURED_IMAGE_SIZES` |
| **#12 ListPageHeader** | ✅ | `7b39321` | `blog/list-page-header.tsx`，统一 h1 字号 + 行高 |
| **#13 列表页用 ListPageHeader** | ✅ | `ac64bbd` | category × 2、tag × 2、archive × 2 共 6 文件，删 -58/+36 行 |
...
- `bun run lint` ✅
- `bunx vitest run` → 74 files / 1379 tests / 0 fail
- `bun run test:coverage` → 100% / 99.59% / 100% / 100%

### 计数变化

第一波后剩 58 项；第二波闭合的审计项：

- **A-05** raw input className 重复 → 收回 Input/Textarea
- **A-06** raw button className 重复 → 收回 Button（taxonomy-manager-form）
- **A-07/C-12** admin 空状态纯文本 → mcp-tokens + posts-grid 用 EmptyState（其余裸 `<p>` 因为表格 colSpan / 主副双行结构维持原样）
- **A-18** site-identity raw textarea → Textarea
- **B-10** 列表页 h1 三套字号不齐 → ListPageHeader
- **B-12** EmptyState 无 CTA → 加 action slot + 8 处接入
- **B-13** Image fill sizes 重复 → FeaturedImage + 常量
- **S-16** 共享 primitives 缺失 → 新增 `Textarea` / `EmptyState (ui)` / `FormField` / `FeaturedImage` / `ListPageHeader` 5 个

剩 **~50 项**未处理，落到第三波（系统加固：S-01 token 桥层、S-03 globals.css 拆分、S-08 print stylesheet、A-10/A-11/A-12 字段反馈链路、A-15 CommandPalette 升级、S-07 i18n）。

### 第二波之外的发现

- `EmptyState` 的 `<Link>` 直接走 next/link，blog/admin 都能用，无需在两个 variant 间分叉行为
- `taxonomy-manager-table` / `admin-posts-list-view` 的空状态在 `<tbody>` 里，必须 `<td colSpan>` 跨表格列，`EmptyState` 无法直接塞入 — 此类模式需独立的"行内空态"模式或 wrapper-around-table 改造，留给第三波
- `media-library` 空状态有"主标题 + 副文本"双行（"暂无媒体文件 / 通过文章编辑器上传图片"），单 message 字段不够；如要收，需扩展 EmptyState 的 description prop。当前不动
- `ai-agents-manager` 空态有"创建代理" CTA 触发本地 modal handler（不只是 href 跳转），EmptyState 当前只支持 Link href。要收需让 action 接受 `onClick` 或 children 渲染
- post-card 的 featured image 用 `<Link>` 做整图 overlay，结构是 `div > Link > Image`；FeaturedImage 自身就是 `div > Image`，所以这里只复用 `FEATURED_IMAGE_SIZES` 常量。如要进一步收敛，需让 FeaturedImage 支持 `linkHref` prop 内部包 Link
- ListPageHeader 暂不含 breadcrumb（原计划 "h1+meta+breadcrumb"），blog 当前没有 breadcrumb 设计。如未来加，不破坏接口

---

## 执行记录 — 第三波（2026-06-09 完成）

「6.3 第三波：系统加固」全部 12 项落盘 + 1 文档 commit = **13 个原子化提交**（计划 14，#7+#8 合并到一个 commit + #8 改为 helper 抽测试）。全部 pre-commit（typecheck + lint + 1395 测试）；Coverage 100% statements / 99.6% branches / 100% functions / 100% lines。

| 项 | 状态 | Commit | 改动摘要 |
|----|------|--------|----------|
| **#1 S-03 拆分 globals.css** | ✅ | `d837101` + `bc31a2d` | 切成 8 个 `src/app/styles/*.css`：tokens / utilities / prose / blog-shell / zed-footer / blog-preview / reference-card / misc；globals.css 从 1252 行减到 73 行（@font-face + base + @imports） |
| **#2 S-08 print stylesheet** | ✅ | `acc1c37` | 新建 `styles/print.css`：隐藏 sidebar/global-bar/zed-footer/cross-divider/page-wrapper 装饰；prose 11pt + 显示链接 href + page-break-inside avoid |
| **#3 S-01a primitive audit** | ✅ | （审计） | 只发现 2 处：`blog/empty-state.tsx` re-export + `blog/content-image-lightbox.tsx` 用 `ImageLightbox`；audit 本身无 commit |
| **#4 S-01 token 桥层** | ✅ | `760a73b` | tokens.css 加 `.blog-shell { --background: ... }` + `.dark .blog-shell { ... }` 把 admin HSL token 在 blog scope 下重绑成 cream/dark blog 等价值，primitive 不动 |
| **#5 A-10 PostForm spinner** | ✅ | `773c95c` | 保存按钮加 `<Loader2 animate-spin>` 与 ai-agent-form 风格对齐 |
| **#6 A-12 TaxonomyForm 校验** | ✅ | `2ad7a70` | 新建 `lib/slug-validation.ts`（7 单测）+ TaxonomyForm 接 `existingSlugs`/`currentSlug`，slug onBlur 校验格式 + 重复，错误展示在 FormField，错误时禁用保存 |
| **#7 A-15 CommandPalette nav** | ✅ | `8c3cb66` | 加 `NAV_COMMANDS` 13 项 + `useMemo` 过滤 + `handleSelect` 索引分配 + "导航"/"文章" 两 section grouping + placeholder 改"搜索文章或跳转页面..." |
| **#8 A-15 helper 抽出** | ✅ | `48a5a2b` | 把 `filterCommandsByQuery` 抽到 `lib/command-filter.ts`（5 单测，通用类型 `FilterableCommand`） |
| **#9 A-11 upload queue** | ✅ | `8bc2047` | useMediaUpload 改 `UploadItem` 队列（pending/uploading/success/error）+ `dismissQueue`；新建 `MediaUploadQueue` 组件渲染 per-file 列表（含 spinner / check / x 图标 + 错误消息内联） |
| **#10 S-07 i18n scaffold** | ✅ | `c28be63` | 新建 `lib/i18n/index.ts`：扁平 dict `zh-CN`（22 个 key）+ 类型安全 `t()` helper（missing key 返回 key 字符串）+ 4 单测 |
| **#11 S-07 blog strings 迁入** | ✅ | `d81e486` | theme-toggle "浅色"/"深色"/"跟随系统"/"切换主题"、blog-footer COLUMNS headings + "回到顶部" 全部走 `t()` |
| **#12 S-07 admin strings 迁入** | ✅ | `9d81e04` | shell.tsx `PAGE_TITLES` 改 `PAGE_TITLE_KEYS: Record<string, TranslationKey>`，title 走 `t(key)` |
...
- `bun run typecheck` ✅
- `bun run lint` ✅
- `bunx vitest run` → 77 files / 1395 tests / 0 fail
- `bun run test:coverage` → 100% / 99.6% / 100% / 100%（门槛 95%）
- 新增测试 16 个：`slug-validation` 7 + `command-filter` 5 + `i18n` 4

### 计数变化

第二波后剩 ~50 项；第三波闭合的审计项：

- **S-03** globals.css 1259 行单文件 → 拆成 entry + 8 个分文件
- **S-08** print stylesheet 只 1 行 → 完整 stylesheet
- **S-01** Token 双轨制无桥接 → `.blog-shell` scope 重绑 admin HSL token 到 blog 等价值，blog 表面复用 admin primitive 直接派生 cream
- **S-11** legacy `.blog-footer` / `--breakpoint-desktop` 死代码（第一波已删）+ 本轮再清 `--brand-hue` fallback 路径无变化
- **A-10** PostForm 保存按钮无 spinner → Loader2
- **A-11** Media library 上传只单行文本 → per-file queue 含 status icon + 错误消息
- **A-12** TaxonomyManager slug 无内联校验 → onBlur + 错误展示 + 禁用保存
- **A-15** CommandPalette 仅搜文章 → +13 个 navigation 命令 + 分组渲染
- **S-07** i18n 完全缺失 → dictionary 框架就位（22 keys），blog/admin 共 5 处接入

剩 ~40 项未处理，主要是：B 系列剩余 P2（B-15 text-wrap pretty、B-17 code 复制按钮、B-20 a11y tabindex 等），A 系列 P2（A-19 sidebar dropdown、A-20 analytics stale、A-23 datetime-picker 等），S 系列剩余（S-05 断点统一、S-06 图标规范、S-09 focus-visible 用 blog accent、S-10 footer inline style、S-15 font preset 泄漏、S-17/S-18 a11y 长尾）。

### 第三波之外的发现

- **CSS 拆分两次尝试**：第一次 Python 脚本切到 :root palette 末尾的 `}` 上，blog-preview 末尾算错位置 → 回滚精切 brace 平衡后通过。教训：纯字节切割 CSS 必须用 brace counting 验证，而不是按"comment 边界 = section 边界"猜。
- **vitest 并发污染**：拆 globals 后第一次 `vitest run` 偶发 6 个 `category.test.ts` 失败（`Unexpected token 'C'` JSON 解析），但单文件跑 12/12 过，全跑重试也过。配置 `pool:"threads"`, `isolate:false`, `maxConcurrency:20` 的副作用，与本轮改动无关 — 留作潜在 retro 项。
- **S-01 桥层只重绑 token，不改 primitive**：blog 表面用 `<AlertDialog>` 现在显示 cream 背景而非灰白，但 `.blog-preview-theme` 自重绑 palette 不受影响。回归风险低，但任何"硬编码 zinc-950" 仍绕过桥层（image-lightbox/alert-dialog overlay 的 backdrop），下一波清理。
- **CommandPalette `useMemo posts`** 是 eslint react-hooks 强制要求：`handleSelect` 依赖 `posts`，否则报警告。一并改了 `setActiveIndex(0) on each keystroke` 避免 nav→posts 切换时索引越界。
- **i18n 渐进式**：dictionary 现 22 key，只迁了 7 处调用（theme-toggle 3、blog-footer 5、admin shell 12），其余还散在各组件里。意图是"开个口子"，不是一次全迁。`t()` 在服务端组件也能跑（纯函数），未来如要 client-side locale switcher，加 React Context 包一层即可。
- **`exactOptionalPropertyTypes`** 这轮再次踩坑：TaxonomyForm 把 `currentSlug?: string` 当作"可选 + 不传 undefined"，调用方 `find()?.slug` 返 `undefined`，必须条件展开 `{...(x ? { prop: x } : {})}`。这是项目 tsconfig 的严格模式，所有第三方 props 接入都要注意。

---

## 执行记录 — 第四波（2026-06-10 完成）

「6.4 第四波：选做的设计资产」全部 14 个原子化提交落盘 + 1 文档 commit = **15 个原子化提交**。全部 pre-commit（typecheck + lint + 1406 测试）；Coverage 99.89% statements / 99.45% branches / 100% functions / 100% lines（门槛 95%）。

| 项 | 状态 | Commit | 改动摘要 |
|----|------|--------|----------|
| **#1 dark image dim** | ✅ | `b81a84b` | `.dark .blog-content/.blog-featured-image img` 加 `filter: brightness(0.85) contrast(1.05)` + hover 还原；尊重 `prefers-reduced-motion` |
| **#2 text-wrap** | ✅ | `9caaa96` | prose-firefly headings `text-wrap: balance` + `<p>` `text-wrap: pretty`，CJK 长标题不再尾行孤字 |
| **#3 inline code + footer** | ✅ | `40fcd0a` | 行内 code padding 0.2em/0.4em + font-size 0.875em + radius 3px；blog-footer 80 行 inline style 全抽到 `.zed-footer-copyright` / `.zed-footer-back-to-top` 含 hover |
| **#4 focus-visible accent** | ✅ | `4f63b9a` | `.blog-shell *:focus-visible` 用 `var(--blog-accent)` override admin `--ring` |
| **#5 reading progress** | ✅ | `ad6e9f6` | 新建 `<ReadingProgress>` fixed top 1px，按 `documentElement.scrollHeight - innerHeight` 计算，passive listener，reduced-motion 直接不显示 |
| **#6 code block enhance** | ✅ | `9dda924` | `renderMarkdown` 给 `<pre>` 加 `class="blog-code-block" data-lang`；新建 `<CodeBlockActions>` 客户端组件用 `createPortal` 把 copy 按钮 + lang 标签注入每个 pre；hover 才显示，clipboard 失败静默 |
| **#7 article TOC** | ✅ | `b389169` | 新建 `extractToc(markdown)` 纯函数（5 单测 含 CJK + fence skip + inline strip）；`<ArticleToc>` IntersectionObserver `-40% 0% -55% 0%` 高亮当前段；< 3 entries 直接不渲染；slug page 改 flex sibling 布局 |
| **#8 comment permalinks** | ✅ | `3af0106` | CommentItem 加 `id="c-{id}"` + `<a href="#c-{id}">` 包时间戳 + `scroll-mt-12` |
| **#9 comment form** | ✅ | `80fa5b2` | 新建 `createComment` entity（2 单测）+ `/api/comments` POST route（admin-only auth check）+ `<CommentForm>` 客户端组件；Comments 0 评论时改为"暂无评论"+ form，访客侧显示"留言功能仅站长开启"placeholder |
| **#10 OG image route** | ✅ | `acea540` | `/api/og` Edge Runtime route 用 `next/og` ImageResponse 返 1200×630 cream + 蓝点 brand strip；`max-age=31536000 immutable` |
| **#11 OG fallback wire** | ✅ | `8217859` | post `generateMetadata` 没 featured_image 时用 `/api/og?title=&subtitle=` 兜底 |
| **#12 archive year heatmap** | ✅ | `b1d5262` | `<ArchiveHeatmap>` 12 列网格 × 多年份行；intensity 用 `color-mix(in oklab, --blog-accent var(--cell-intensity), --blog-separator)`；archive year 页 (`/archive/2026`) 自动展示当年热力图 |
| **#13 wave-3 review #1** | ✅ | `026b504` | （已含在第三波）slug helper CJK Unicode 范围扩展，与 `slugify()` 对齐 + slugify→validateSlug 一致性回归 |
| **#13 wave-3 review #2** | ✅ | `d483594` | （已含在第三波）upload 并发 drop 拒绝 + `uploadingRef` 同步检查，try/finally 保证 ref/state 清理 |
| **#14 archive index** | ✅ | `33ac0b7` | 新建 `/archive` index 路由，全部年份 heatmap + ListPageHeader + EmptyState |
...
- `bun run typecheck` ✅
- `bun run lint` ✅
- `bunx vitest run` → 77 files / 1406 tests / 0 fail
- `bun run test:coverage` → 99.89% / 99.45% / 100% / 100%（门槛 95%）
- 新增 7 测试：extractToc 5 + createComment 2

### 计数变化

第三波后剩 ~40 项；第四波闭合的审计点 + 升级机会：

- **B-15** prose `text-wrap`
- **B-16** 行内 code padding/font-size
- **B-19** footer inline style → CSS class
- **B-20**（部分）dark image 处理
- **S-09** focus-visible 用 blog-accent
- **S-10** footer inline style 抽出
- **S-13/B-08（部分）**：评论形态保留 left-border 但加 permalink
- **B-06** 评论无 form → admin form 接入（B-23 0 评论返 null 同时修）
- **B-14** 评论锚点 permalink
- **B-17** 代码块 copy + 语言
- **#12 TOC**（新增能力）
- **#13 代码块增强**（新增能力）
- **#14 OG 自动生成**（新增能力）
- **#15 Reading progress**（新增能力）
- **#16 Tag/Category 热力图**（升级为 archive 热力图 + index 入口）
- **#17 dark image brightness**
- **#18 评论表单 + permalink**

剩 ~20 项 P2 长尾：A-19 sidebar dropdown / A-20 analytics stale / A-22 sticky bar z-index / A-23 datetime-picker / A-24/A-25 / B-04 后续验证 / B-07 圆角统一 / B-11 tag pill hover 不用 opacity / B-21 byline 顺序 / B-22 footer 5 列 / B-25 noise will-change / S-05 断点统一 / S-06 图标规范 / S-12 tag palette dedup / S-14 blog-preview 镜像 / S-15 font preset 泄漏 / S-17 theme-toggle aria / S-18 noscript fallback。

### 第四波之外的发现

- **`@vercel/og` 在 Railway**：项目用 Railway 部署 + Next.js node runtime，`next/og` Edge route 已被 build 支持（next 16 默认 bundle satori）。无需额外字体——内置 Roboto 子集 + 系统 fallback 对中英文 OG title 已够用。如未来想要 IBM Plex / 自定义品牌字体，需要在 route 里 `fetch` woff2 并通过 `fonts:` 传入。
- **TOC 渲染层**：服务端 `extractToc` 用纯字符串扫描而非走 marked AST。理由：marked 的 lexer 同步出 token 树后还要 `parser.parseInline` 才拿得到纯文本，重活；行扫 + fence detection 已足够正确（4 单测覆盖含 CJK / inline strip / fence skip）。
- **CommentForm 当前是 admin-only**：API route 显式 auth check 拒未登录 POST。这是有意为之的最小可工作版 —— 开放评论需要 spam protection / captcha / moderation queue，不属于本波"内容呈现升级"。下一波（如有）可加 anonymous mode + 邮箱验证 + 评论 hold 状态。
- **Archive 热力图 oklab 插值**：`color-mix(in oklab, accent X%, separator)` 在浅色和深色模式下都能产生感知线性的强度。比 `rgb` 插值在低强度区域颜色保持更稳。
- **OG 图缓存**：`Cache-Control: public, max-age=31536000, immutable` — query string 已是 cache key（title + subtitle 唯一确定输出）。如果换模板要"破缓存"，给 URL 多带个 `?v=2` 即可。Twitter / Facebook 各自有内部缓存（24h 起步），无法主动 purge，所以模板版本号最好稳定。
- **Reading progress 全文档进度而非文章进度**：用 `documentElement.scrollHeight` 而不是 article ref 的高度。读者直觉是"页面上还剩多少"——含评论和 footer，与 medium/substack 一致。如果要严格"文章进度"，传一个 ref 进来 measure。
- **#7 ArticleBody flex sibling**：把 `<ArticleBody>` 包在 `<div className="flex gap-8">` 里 + `<ArticleToc>` 作为兄弟。文章内 indent 没全部改 +2（保持改动可读 diff），typecheck 通过即可。后续如果做 Plan-mode 重排，再统一缩进。
