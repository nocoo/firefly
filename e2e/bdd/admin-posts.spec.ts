...
test.describe("Feature: Admin posts list", () => {
  test("Given /admin/posts is requested, When I open it, Then the AdminShell h1 (文章) is visible", async ({
    ...
    // AdminShell h1 — i18n key "admin.page.posts" → "文章"
  ...
  test("Given /admin/posts renders in list mode, When I view the list, Then the <table> is visible and per-state surfaces 未找到文章 (empty) OR the first 编辑 row link (non-empty)", async ({
    ...
    // admin-posts-list-view.tsx:198 always renders <table>; empty branch
    ...
    if (editCount === 0) {
      // Empty-state copy from L211. Scoped to <table> so unrelated
      // 未找到文章 text elsewhere on the page cannot satisfy.
      await expect(
  ...
  test("Given /admin/posts renders, When I view the header actions, Then the 新建文章 link to /admin/posts/new is visible", async ({
    ...
    // admin-posts-client.tsx:161-166 renders a Next.js <Link> with text
...
test.describe("Feature: Admin post new editor", () => {
  test("Given /admin/posts/new is requested, When I open it, Then the AdminShell h1 (文章) and 文章标题 placeholder input are visible", async ({
    ...
    // /admin/posts/new is wrapped by the admin layout, so the AdminShell h1
    // stays "文章" (page.subtitle changes via PageSubtitleContext but h1
    // does not). exact:true to avoid 文章-prefix substring matches.
    await expect(
      page.getByRole("heading", { level: 1, name: "文章", exact: true }),
    ).toBeVisible({ timeout: 10_000 });

    // post-form.tsx:273 — title <Input> placeholder is exactly "文章标题".
    await expect(
  ...
  test("Given /admin/posts/new renders, When I view the editor, Then at least one content textarea (#content-mobile or #content-desktop) is attached", async ({
    ...
    // post-form-content-editor.tsx mounts BOTH textareas simultaneously —
  ...
  test("Given /admin/posts/new renders, When I view the form action area, Then 创建文章 + 取消 are visible inside the form AND 删除文章 has 0 count (new mode never exposes delete)", async ({
    ...
    // post-form.tsx:386-391 wraps editorFields (including action area) in
    // <form>. Scope to it so unrelated page-level buttons cannot satisfy.
    ...
    // post-form.tsx:357-367 — the {isEditing && ...} branch renders 删除文章.
    // In new mode (post=undefined) that button must never appear inside the
    // form. Reverse-assert count to guard against a future regression that
    // accidentally surfaces delete in new mode.
    await expect(
  ...
  test("Given /admin/posts/new renders, When I fill a unique-prefixed title, Then the title input has that value AND the slug input is auto-synced to the slugified value", async ({
    ...
    // Unique prefix avoids cross-run pollution; we never submit, so no DB
    // write happens — the value is verified entirely client-side.
    const ts = Date.now();
    ...
    // Primary contract from the original test: title input must hold the
    // exact value the user typed.
    await expect(titleInput).toHaveValue(uniqueTitle);

    // Additional contract (documented in commit body / mapping): in new
    // mode, post-form.tsx:152-158 auto-syncs slug from slugify(title) as
    // long as the slug field has not been manually edited. toHaveValue
    // polls until the React state propagates — no fixed timeout fallback.
    await expect(
...
test.describe("Feature: Admin post edit page", () => {
  test("Given /admin/posts has at least one post, When I click the first row's 编辑 link, Then pathname matches /admin/posts/<id>/edit with a non-empty uuid segment", async ({
    ...
    // Structural assertion — pathname cannot be a single literal here, so
    // pin the prefix + suffix + non-empty uuid segment instead of a regex
    // match on page.url(). This matches the path-level guard discipline
    // used elsewhere in the BDD suite.
    ...
    // Sanity: returned helper id matches the URL segment.
  ...
  test("Given an existing post edit page renders, When I view the content editor, Then at least one content textarea (#content-mobile or #content-desktop) is attached", async ({
    ...
    // Same responsive dual-mount pattern as the new editor — isAttached, not
    // isVisible. post-form-content-editor.tsx:171-205.
    const contentArea = page.locator(
      "textarea#content-mobile, textarea#content-desktop",
    );
    expect(await contentArea.count()).toBeGreaterThan(0);
  ...
  test("Given an existing post edit page renders, When I view the status/category/tag controls, Then select#status, select#category, and the 标签 label are visible", async ({
    ...
    // post-form-fields.tsx PostStatusCategoryRow renders <Select id="status">
    // (L102-111) and <Select id="category"> (L118-129). CSS selectors are
    // the stable signature — Select wraps a native <select>.
    await expect(page.locator("select#status")).toBeVisible();
    await expect(page.locator("select#category")).toBeVisible();

    // PostTagsField renders <label>标签</label> at L183. The admin sidebar
    // also surfaces "标签" as a <span> nav item; scoping to <label> avoids
    // that collision.
    await expect(
  ...
  test("Given an existing post edit page renders, When I view the form action area, Then 更新文章 (submit) and 删除文章 are visible inside the form", async ({
    ...
    // Scope to <form>; in edit mode the {isEditing && ...} branch
  ...
  test("Given an existing post edit page renders, When I click 取消, Then pathname is exactly /admin/posts", async ({
    ...
    // post-form.tsx:350-356 — cancel button is a <button type="button">
    // that calls router.push("/admin/posts"). Scope to <form> so other
    // 取消 buttons (e.g. ConfirmDialog cancel) cannot satisfy.
    const form = page.locator("form");
  ...
  test("Given a nil-uuid edit URL is requested, When I open it, Then the HTTP response status is exactly 404", async ({
    ...
    // src/app/admin/posts/[id]/edit/page.tsx:30 calls notFound() when
    // getPostById returns null. The nil uuid 00000000-... never matches
    // a real post by construction.
